...
Component({
  ...
  methods: {
        ...
        wx.downloadFile({
          ...
          success: function(res) {
            ...
            if (res.statusCode === 200) {
              ...
              that.sharePosteCanvas(productSrc, codeSrc, imgInfo, shopSrc, avaSrc);
            } else {
              wx.showToast({
                title: '二维码下载失败！',
                icon: 'none',
    ...
    //canvas绘制分享海报
    sharePosteCanvas: function (avaterSrc, codeSrc, imgInfo, shopSrc, avaSrc) {
      //console.log(imgInfo);
      wx.showLoading({
        title: '生成中...',
        mask: true,
      ...
      query.select('#canvas-container').boundingClientRect(function(rect) {
        ...
        //标题
        if (that.data.title) {
          const CONTENT_ROW_LENGTH = width*2*0.85/14; // 正文 单行显示字符长度
          let [contentLeng, contentArray, contentRows] = that.textByteLength((that.data.title), CONTENT_ROW_LENGTH);
          ctx.setTextAlign('left');
          ctx.setFillStyle('#333333');
          ctx.font = 'normal bold 14px sans-serif';
          let contentHh = 18 * 1;
          for (let m = 0; m < contentArray.length; m++) {
            ctx.fillText(contentArray[m], width * 60 / 750, width * 480 / 750+ contentHh * m);
          }
          titleM = contentArray.length;
        }
        //console.log(titleM)
        //设置地址
        if (that.data.addr) {
          ...
          const CONTENT_ROW_LENGTH = width * 2 * 0.85 / 14; // 正文 单行显示字符长度
          let [contentLeng, contentArray, contentRows] = that.textByteLength((that.data.addr), CONTENT_ROW_LENGTH);
          ctx.setTextAlign('left');
          ctx.setFillStyle('#333');
          ctx.font = 'normal normal 14px sans-serif';
          let contentHh = 20 * 1;
          //console.log(width);
          for (let m = 0; m < contentArray.length && m < 0.62*width/20; m++) {
            if (m == contentArray.length || m == parseInt(0.62 * width / 20)){
              contentArray[m] = contentArray[m].substring(0, contentArray[m].length - 3) + "...";
            }
            ctx.fillText(contentArray[m], width * 60 / 750, width * (650 - 40 * (3 - titleM) ) / 750 + contentHh * m);
          }
        }
        ...
        if (that.data.productname) {
          ...
          let contentHh = 22 * 1;
          for (let m = 0; m < contentArray.length; m++) {
            ctx.fillText(contentArray[m], width * 120 / 750, width * 390 / 750 + contentHh * m);
          }
        }
        ...
        //绘制二维码
        if (codeSrc) {
          ctx.drawImage(codeSrc, width * 50 / 750, width*1235/750, width / 5, width / 5)
          ctx.setFontSize(10);
          ctx.setFillStyle('#000');
        ...
        success(res) {
          wx.getSystemInfo({
            success(res2) {
              ...
              var imgHeight = (res2.windowWidth * 0.75 / ratio);
              that.setData({
                imgHeight: imgHeight
              })
              cb(imgHeight);
